...
public class SideEffectsService {
    ...
    public Map<String, Integer> getAllSideEffectsCounts() {
        List<SideEffectCount> allCounts = sideEffectsRepository.findAll();
    ...
    @Transactional
    public void addSideEffectsCount(List<SideEffectType> inputSideEffectTypes) {
        ...
        if (foundSideEffectCounts.size() != inputSideEffectTypes.size()) {
            for (SideEffectType type : inputSideEffectTypes) {
                SideEffectCount target = new SideEffectCount(type);
